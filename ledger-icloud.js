...
const App = {
    ...
    async handleRegister() {
        ...
        if (!u || !p) return this.showMessage(this.constants.ELEM_ID.loginMsg, '请输入用户名和密码');
        if (this.state.users[u]) return this.showMessage(this.constants.ELEM_ID.loginMsg, '用户已存在');
    ...
    async handleLogin() {
        ...
        const p = $id(this.constants.ELEM_ID.password).value;
        if (!u || !p) return this.showMessage(this.constants.ELEM_ID.loginMsg, '请输入用户名和密码');
        if (!this.state.users[u]) return this.showMessage(this.constants.ELEM_ID.loginMsg, '用户不存在');
